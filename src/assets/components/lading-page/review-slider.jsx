      ...
      <div className="overflow-hidden">
        <Slider ref={sliderRef} {...settings}>
          {productData?.products?.map((item) => {
            return item.reviews.map((it, index) => {
              // console.log(`${item.id}-${index}`);
              const formattedDate = new Date(it.date).toLocaleDateString(
                "en-US",
                {
                  year: "numeric",
                  month: "long",
              ...
              return (
                <div key={`${item.id}-${index}`} className=" pl-5">
                  <div className="px-8 py-7 border border-black/10 rounded-[20px]">
                    <p className="text-base leading-[22px] mb-[14px]">
                      Rating: {it.rating}/
                      <span className="text-black/60">5</span>
                    </p>
                    <h3 className="font-Satoshi-bold text-[20px] leading-[1] mb-3">
                      {it.reviewerName}
                    </h3>
                    <p className="text-base leading-[22px] text-black/60 mb-2">
                      "{it.comment}"
                    </p>
                    <p className="text-sm ">Commented on {formattedDate}</p>
                  </div>
                </div>
              );
            });
